...
public class ImpiegatoServlet extends HttpServlet {
	...
	public void init() throws ServletException {
		...
		System.out.print("ImpiegatoServlet. Apertura connessione DB...");
	...
	@Override
	public void destroy() {
		System.out.print("ImpiegatoServlet. Chiusura connessione DB...");
		try {
			dao.closeConnection();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		System.out.println("ImpiegatoServlet. Invocato metodo doGet");
		...
		int idImpiegato = -1;
		try {
			idImpiegato = am.getSub(jwt);
		} catch (NumberFormatException e) {
			e.printStackTrace();
			response.setStatus(400);
			resJsonObject = new JSONObject(new MessaggioRisposta("ID impiegato invalido"));
			response.getWriter().append(resJsonObject.toString());
			return;
		}
		...
		JSONObject impiegatoJson = null;

		impiegatoJson = new JSONObject(impiegato);

		out.print(impiegatoJson.toString());
		out.flush();
	...
	protected void doPut(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		System.out.println("ImpiegatoServlet. Invocato metodo doPut");
		...
		try {
			res = dao.modificaImpiegato(impiegato, parametro, nuovoValore);
		...
		if (res > 0) {
			...
			resJsonObject = new JSONObject(new MessaggioRisposta("Modifica apportata con successo"));
			response.getWriter().append(resJsonObject.toString());
		} else {
			response.setStatus(404);
